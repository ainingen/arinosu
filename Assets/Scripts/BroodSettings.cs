...
public class BroodSettings : ScriptableObject
{
    ...
    [Tooltip("いちばん奥からこれだけ浅ければ、女王はさらに奥へ寄る（深さの割合。13-15）")]
    public float queenDeepTolerance = 0.05f;
    ...
    {
        if (item == null) return preferredDepthLarva;
        switch (item.stage)
        {
            case BroodStage.Egg:
                return preferredDepthEgg;
            case BroodStage.Larva:
                return item.size < youngLarvaSize ? preferredDepthEgg : preferredDepthLarva;
            default:
                return preferredDepthPupa;
        }
    }
